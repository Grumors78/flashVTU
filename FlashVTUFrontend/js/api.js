...
function getToken() {
  return localStorage.getItem('fvtu_token');
}

function setToken(token) {
  localStorage.setItem('fvtu_token', token);
}

function clearToken() {
  localStorage.removeItem('fvtu_token');
  localStorage.removeItem('fvtu_user');
}

function getStoredUser() {
  const raw = localStorage.getItem('fvtu_user');
  return raw ? JSON.parse(raw) : null;
}

function setStoredUser(user) {
  localStorage.setItem('fvtu_user', JSON.stringify(user));
}
...
const api = {
  ...
  createFundingAccount: (amount) => apiRequest('/wallet/create-funding-account', { method: 'POST', body: { amount } }),
